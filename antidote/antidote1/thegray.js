...
function updateTheGray(delta) {
    ...
    window.objects.forEach(object => {
        if (object.userData && object.userData.isTheGray && object.userData.health > 0) {
            ...
            if (object.userData.aiState === 'alert') {
                // In alert state, move directly towards the player
                const direction = new THREE.Vector3().subVectors(playerPosition, grayPosition).normalize();
                const moveAmount = currentSpeed * delta;
                object.position.x += direction.x * moveAmount;
                object.position.z += direction.z * moveAmount;

                // Also, make the enemy look at the player
